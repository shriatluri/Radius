...
const STATUS_MAP: Record<number, new (code: string, message: string, body?: RadiusErrorBody) => RadiusError> = {
  400: BadRequestError,
...
export async function raiseForStatus(response: Response): Promise<void> {
  ...
  let body: RadiusErrorBody | undefined;

  try {
    body = (await response.json()) as RadiusErrorBody;
